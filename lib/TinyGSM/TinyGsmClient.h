...
public:
  ...
  bool eraseForwarding(int fwReason) {
    sendAT(GF("+CCFC="), fwReason, GF(",4"));
    return waitResponse(180000L) == 1;
  }

  bool registerForwarding(int fwReason, const String& number) {
    sendAT(GF("+CCFC="), fwReason, GF(",3,\""), number, GF("\""));
    return waitResponse(180000L) == 1;
  }

  bool registerForwarding(int fwReason, const String& number, int fwTime) {
    sendAT(GF("+CCFC="), fwReason, GF(",3,\""), number, GF("\",,,,,"), fwTime);
    return waitResponse(180000L) == 1;
  }

  FwStatus getForwardingStatus(int fwReason) {
    FwStatus res;

    sendAT(GF("+CCFC="), fwReason, GF(",2"));
    if (waitResponse(180000L, GF(GSM_NL "+CCFC:")) != 1) {
      res.number = "ERROR";
    }
    res.enabled = stream.readStringUntil(',').toInt();

    if (streamSkipUntil('"', 5000L)) {
      res.number = stream.readStringUntil('"');
    } else {
      res.number = "";
    }
...
private:
  ...
  bool streamSkipUntil(char c, uint32_t timeout) {
